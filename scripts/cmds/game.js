...
function getScoreboard(game) {
    let board = "📋 [ قائمة اللاعبين ]\n━━━━━━━━━━━━━━━━━\n";
    for (let uid in game.players) {
        let p = game.players[uid];
        board += `♢ @${p.name} ${p.fruit.repeat(p.lives)}\n`;
    }
    return board;
}
...
module.exports.onStart = async ({ api, event, usersData }) => {
    ...
    if (cmd.includes('done')) {
        ...
        let targetName = "لاعب";
        let targetGender = 2; // 2 means male by default, 1 means female
        
        try { 
            if(usersData) { 
                const userData = await usersData.get(targetID); 
                targetName = userData.name || "لاعب"; 
                targetGender = userData.gender || 2;
            } 
        } catch (e) {}
        ...
        game.players[targetID] = { name: targetName, lives: 3, fruit: fruit, gender: targetGender };
    ...
    if (cmd.includes('go')) {
        ...
        game.status = 'playing';
        let scoreboard = getScoreboard(game);
...
module.exports.onChat = async ({ api, event }) => {
    ...
    if (game.status === 'playing' && game.currentQuiz && game.players[senderID]) {
        ...
        if (isCorrect) {
            ...
            let pGender = game.players[senderID].gender;
            let winWord = (pGender === 1) ? "جبتيها لاصقة نتي الأولى" : "جبتيها لاصقة نتا الأول";
            
            api.setMessageReaction("✅", messageID);
            return api.sendMessage(`👏 برافوووو @${game.players[senderID].name}! ${winWord}.\n\n⚔️ دابا اكتب سمية الضحية اللي بغيتي تحيد ليه فاكهة (اكتب غير أول حروف من سميتو):`, threadID, messageID);
        }
    }
    ...
    if (game.status === 'attacking' && senderID === game.attacker) {
        ...
        if (targetUID) {
            ...
            // تحديد الجنس للأفعال
            let isAttackerFemale = (attackerP.gender === 1);
            let hitVerb = isAttackerFemale ? "حيدات" : "حيد";
            
            let isTargetFemale = (targetP.gender === 1);
            let loseVerb = isTargetFemale ? "خسرات وتقصات" : "خسر وتقصى";

            let msg = `💥 بـــــووووم!\n@${attackerP.name} ${hitVerb} فاكهة لـ @${targetP.name} 🔪\n\n`;
            
            if (targetP.lives <= 0) {
                msg += `💀 @${targetP.name} ${loseVerb} من اللعبة!\n\n`;
            ...
            if (remainingPlayers.length === 1) {
                ...
                let isWinnerFemale = (winnerP.gender === 1);
                let winPhrase = isWinnerFemale ? "انتي هي الفائزة" : "انت هو الفائز";
                
                msg += `🎉🎊 مبروووووووك @${winnerP.name} ${winPhrase} فهاد اللعبة! احتافظتي على الفواكه ديالك 🏆👏\n\n_________________\nمطور البوت 𝗦𝗜𝗠𝗢`;
                game.status = 'idle';
                return api.sendMessage(msg, threadID);
            }

            // إضافة القائمة المحدثة بعد الهجوم
            msg += getScoreboard(game);
